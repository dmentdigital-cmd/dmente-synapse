...
export function LoginScreen({ onAuthenticated }: Props) {
  ...
  return <main className="auth-screen"><section className="auth-card panel-card"><img className="auth-logo" src="/assets/logo-dmente.png" alt="Dmente Digital" /><span className="eyebrow">ACCESO PROTEGIDO</span><h1>Entrar a Synapse</h1><p>Accede a tu centro de operaciones y a tus dominios personales.</p><form onSubmit={submit}><label>Usuario<input value={username} onChange={(event) => setUsername(event.target.value)} autoComplete="username" /></label><label>Contraseña<input value={password} onChange={(event) => setPassword(event.target.value)} type="password" autoComplete="current-password" /></label>{error && <div className="auth-error" role="alert">{error}</div>}<button className="auth-submit" type="submit" disabled={submitting}>{submitting ? 'Verificando...' : 'Iniciar sesión'} <ArrowRight size={17} /></button></form><small className="auth-note"><LockKeyhole size={13} /> La sesión se mantiene en una cookie protegida.</small></section></main>
}
