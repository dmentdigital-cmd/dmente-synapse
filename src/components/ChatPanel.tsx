...
type Props = { agent: typeof agents[AgentId]; messages: Message[]; pending: boolean; apiReady: boolean; draft: string; setDraft: (value: string) => void; sendMessage: () => void; minimize: () => void; togglePending: () => void }

export function ChatPanel({ agent, messages, pending, apiReady, draft, setDraft, sendMessage, minimize, togglePending }: Props) {
  return <aside className="chat-panel panel-card">
    ...
    <div className="message-list">{messages.map((message, index) => <div key={`${message.time}-${index}`} className={`message-row ${message.from}`}><div className="message-bubble">{message.text}<span>{message.time}</span></div></div>)}</div>
    <div className="composer"><button className="attach-button" aria-label="Adjuntar archivo"><Paperclip size={17} /></button><input value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && sendMessage()} placeholder="Escribe un mensaje..." /><button className="send-button" aria-label="Enviar mensaje" onClick={sendMessage}><Send size={16} /></button></div>
  </aside>
}
